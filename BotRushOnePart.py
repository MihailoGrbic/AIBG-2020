        ...
        if len(current_game_state.self_info.player_info['parts']) < 1:
            if "singleDigTarget" not in current_game_state.internal_bot_state:
                ...
                if len(diggable_tiles) > 0:
                    (dig_x, dig_y) = find_closest_coordinate((x, y), diggable_tiles, current_game_state.map, current_game_state.other_info)
                    print()
                    diggable_dist = dist((x, y), (dig_x, dig_y))
                    target_dist = dist((x, y), (target_x, target_y))
                    tolerance = 30 / (target_dist + 1)
                    if diggable_dist < tolerance:
